...
public class DeleteTest extends UnitTestBase {
  ...
  public void testDeleteWithForeignKeyIndexes() throws Exception {
    ...
    TableUniqueIndex t5a = metaRepo.table("admin", "t5").uniqueIndexes().get("a");
    assertEquals(0L, t5a.get("x")[0]);
    assertEquals(16L, t5a.get("y")[0]);
    TableMultiIndex t6b = metaRepo.table("admin", "t6").multiIndexes().get("b");
  ...
  public void testDeleteReferencedByRules_MultiTable() throws Exception {
    ...
    execute("alter table t8 add view s (select sum(c) as sc from rev_fk_a);");
  ...
  public void testDeleteReferencedByRules_MultiTableMultipleLevels() throws Exception {
    ...
    execute("alter table t11 add view s (select sum(e) as se from rev_fk_d);");
    execute("alter table t10 add view s (select sum(s.se) as sse from rev_fk_c);");
