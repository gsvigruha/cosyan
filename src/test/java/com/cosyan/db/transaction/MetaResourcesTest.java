...
public class MetaResourcesTest extends UnitTestBase {
  ...
  private Map<String, Resource> resources(String sql)
    ...
    try {
      Iterable<Resource> ress = transaction.collectResources(metaReader, token).lockResources();
  ...
  public void testInsert() throws Exception {
    ...
    assertTrue(res.get("admin.t1").isWrite());
  }
  ...
  public void testDelete() throws Exception {
    ...
    assertEquals(1, res.size());
    assertTrue(res.get("admin.t2").isWrite());
  }
  ...
  public void testUpdate() throws Exception {
    ...
    assertEquals(1, res.size());
    assertTrue(res.get("admin.t3").isWrite());
  }
  ...
  public void testSelectViewForeignKey() throws Exception {
    ...
    assertFalse(res.get("admin.t4").isWrite());
    assertFalse(res.get("admin.t5").isWrite());
  }
  ...
  public void testSelectFromRef() throws Exception {
    ...
    execute("alter table t6 add view s (select sum(b) as b from rev_fk_a);");
    Map<String, Resource> res1 = resources("select a from t6;");
    assertEquals(1, res1.size());
    assertFalse(res1.get("admin.t6").isWrite());

    Map<String, Resource> res2 = resources("select s.b from t6;");
    assertEquals(2, res2.size());
    assertFalse(res2.get("admin.t6").isWrite());
    assertFalse(res2.get("admin.t7").isWrite());
  }
  ...
  public void testModifyWithRules() throws Exception {
    ...
    assertTrue(res1.get("admin.t8").isWrite());
    assertFalse(res1.get("admin.t9").isWrite());

    Map<String, Resource> res2 = resources("delete from t8 where a = 'x';");
    assertEquals(2, res2.size());
    assertTrue(res2.get("admin.t8").isWrite());
    assertFalse(res2.get("admin.t9").isWrite());

    Map<String, Resource> res3 = resources("update t8 set b = 2;");
    assertEquals(2, res1.size());
    assertTrue(res3.get("admin.t8").isWrite());
    assertFalse(res3.get("admin.t9").isWrite());

    Map<String, Resource> res4 = resources("insert into t9 values ('x');");
    assertEquals(2, res4.size());
    assertTrue(res4.get("admin.t9").isWrite());
    assertFalse(res4.get("admin.t8").isWrite());

    Map<String, Resource> res5 = resources("delete from t9 where a = 'x';");
    assertEquals(1, res5.size());
    assertTrue(res5.get("admin.t9").isWrite());

    Map<String, Resource> res6 = resources("update t9 set a = 'x';");
    assertEquals(2, res6.size());
    assertTrue(res6.get("admin.t9").isWrite());
    assertFalse(res6.get("admin.t8").isWrite());
  }
}
